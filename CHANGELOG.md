...
- **Validation reproductible** : 10 tests ciblés passent sous Python 3.11/Docker (authentification, admin, injection du contexte, protocoles legacy/2026, notifications, uploads >4 Mio, limite HTTP et isolation S3) ; le workflow GitHub Actions exécute la même commande avec les dépendances de test figées. Build sans cache réussi, schémas des 40 outils inchangés. Après renouvellement de la clé LLMaaS, 19 contrôles fonctionnels réels passent avec le CLI Go et le serveur SDK 2 : ingestion de 3 documents, extraction d'entités et relations, recherche graphe et vectorielle, déduplication locale/distante, remplacement explicite et cohérence S3/Neo4j/Qdrant.
...
- **Isolation** : un `storage_check(memory_id=...)` ne retourne que les objets du préfixe S3 autorisé et ne divulgue plus les clés appartenant aux autres mémoires.
